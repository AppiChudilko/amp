...
namespace GTANetwork.Sync
{
    internal partial class SyncPed
    {
        ...
        private void UpdateVehicleInternalInfo()
        {
            ...
            {
                LogManager.StreamerLog("Veh IsConvertible1: " + IsConvertible);
                Function.Call(Hash.SET_CONVERTIBLE_ROOF, MainVehicle, IsConvertible);
            }
            else if (!MainVehicle.RoofState. && IsConvertible)
            {
                LogManager.StreamerLog("Veh IsConvertible2: " + IsConvertible);
                Function.Call(Hash.SET_CONVERTIBLE_ROOF, MainVehicle, !IsConvertible);
            }*/
            ...
            //LogManager.StreamerLog("Veh SteeringScale: " + Util.Util.Lerp(MainVehicle.SteeringAngle, SteeringScale, Math.Min(1.5f, TicksSinceLastUpdate / (float)AverageLatency)) + " | " + SteeringScale + " | " + MainVehicle.SteeringScale + " | " + Util.Util.Lerp(MainVehicle.SteeringAngle.ToRadians(), SteeringScale.ToRadians(), Math.Min(1.5f, TicksSinceLastUpdate / (float)AverageLatency)));
            
            //LogManager.StreamerLog("Veh DirtLevel: " + DirtLevel);
            
            MainVehicle.CurrentRPM = VehicleRPM;
            //MainVehicle.DirtLevel = DirtLevel;
            MainVehicle.SteeringAngle = SteeringScale + 0.705883f; //Util.Util.Lerp(MainVehicle.SteeringAngle, SteeringScale, Math.Min(1.5f, TicksSinceLastUpdate / (float)AverageLatency));
            //MainVehicle.SteeringScale = Util.Util.ToRadians(SteeringScale); //Util.Util.Lerp(MainVehicle.SteeringAngle.ToRadians(), SteeringScale.ToRadians(), Math.Min(1.5f, TicksSinceLastUpdate / (float)AverageLatency));
        }
