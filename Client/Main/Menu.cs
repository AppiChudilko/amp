...
namespace GTANetwork
{
    internal partial class Main
    {
        ...
        private readonly Vector3 _delpierroSign = new Vector3(-1358.651f, -793.3086f, 20.24218f);
        ...
        private void RebuildServerBrowser()
        {
            ...
            ThreadPool.QueueUserWorkItem(delegate
            {
                try
                {
                    ...
                    listVerified.Add("appi-rp.com:4490");
        ...
        {
            MainMenu = new TabView("Appi Multiplayer")
            {
                CanLeave = false,
                MoneySubtitle = "AMP " + CurrentVersion
            };
            ...
            {
                _welcomePage = new TabWelcomeMessageItem("Hello ;)", "Welcome to the open beta test Appi Multiplayer ;).~n~We wish you a pleasant game and good mood :3~n~~n~~n~Multiplayer is based on GTA:Network");
            ...
            #region ServerBrowser
            {
                ...
                _connectTab = new TabSubmenuItem("connect", new List<TabItem> { _favBrowser, dConnect,  _Verified, /*_Spotlight, */_serverBrowser, _lanBrowser, _recentBrowser });
            ...
            #region Settings
            {
                ...
                var DebugMenu = new TabInteractiveListItem("Debug", new List<UIMenuItem>());
                {
                    ...
                    {
                        var debugItem = new UIMenuCheckboxItem("Show Vehicle Debug Data", DebugInfo.VehicleDebug);
                        debugItem.CheckboxEvent += (sender, @checked) =>
                        {
                            DebugInfo.VehicleDebug = @checked;
                        };
                        DebugMenu.Items.Add(debugItem);
                    }
            ...
            #region Host
            {
                ...
                if (File.Exists(settingsPath) && Directory.Exists(GTANInstallDir + "\\server\\resources"))
                {
                    ...
                    hostStart.Activated += (sender, args) =>
                    {
                        ...
                        {
                            GTA.UI.Screen.ShowNotification("~b~~h~AMP~h~~w~~n~Leave the current server first!");
                            return;
                        }

                        GTA.UI.Screen.ShowNotification("~b~~h~AMP~h~~w~~n~Starting server...");
                        ...
                        serverName.Activated += (sender, item) =>
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(newName))
                            {
                                GTA.UI.Screen.ShowNotification(
                                    "~b~~h~AMP~h~~w~~n~Server name must not be empty!");
                                MainMenu.TemporarilyHidden = false;
                                return;
                            }
...

                    {
                        ...
                        serverName.Activated += (sender, item) =>
                        {
                            ...
                            {
                                GTA.UI.Screen.ShowNotification(
                                    "~b~~h~AMP~h~~w~~n~Invalid input for player limit!");
                                MainMenu.TemporarilyHidden = false;
                                return;
                            }
...

                    {
                        ...
                        serverName.Activated += (sender, item) =>
                        {
                            ...
                            {
                                GTA.UI.Screen.ShowNotification(
                                    "~b~~h~AMP~h~~w~~n~Invalid input for server port!");
                                MainMenu.TemporarilyHidden = false;
                                return;
                            }
            ...
            #region Quit
            {
                var welcomeItem = new TabTextItem("Quit", "Quit AMP", "Are you sure you want to quit Appi Multiplayer and return to desktop?")
                {
                    CanBeFocused = false
                };
